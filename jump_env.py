...
#            vis_options=gs.options.VisOptions(rendered_envs_idx=list(range(1))),
    ...
    def _resample_commands(self, envs_idx):
        ...
        self.commands[envs_idx, 3] = gs_rand_float(
            *self.command_cfg["jump_height_range"],
    ...
    def _reward_tracking_jump_height(self):
        desired_h = self.commands[:, 3]
        return torch.square(self.base_lin_vel[:, 2]) + torch.where(self.base_pos[:, 2] - desired_h > 0.0, torch.tensor(1), torch.tensor(0))
    
        # return torch.exp(-10.0 * (self.base_pos[:, 2] - desired_h).abs())
